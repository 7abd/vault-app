...
export function VaultProvider({children} : {children:ReactNode}) {
  ...
  const createVaultMetadata =  useCallback( async(password: string) => {


    const { data: { user }, error: userError } = await supabase.auth.getUser();
    if (userError || !user) throw new Error(userError?.message || "User not found");
  
    const verifier = user.user_metadata?.vault_verifier;
  ...
  const unlockVault = useCallback (async(password: string): Promise<boolean> => {
    ...
    const salt = user.user_metadata?.vault_salt;
    const verifier = user.user_metadata?.vault_verifier;
      if(!verifier) {
    ...
    const withDecrypted = useCallback(
      ...
      const value: VaultContextType = useMemo(
        ...
        [isUnlocked, cryptoKey, error, isLoading, lockVault, unlockVault, withDecrypted,clearError,createVaultMetadata,setError]
      );
